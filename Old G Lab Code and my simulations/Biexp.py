#!/usr/bin/env python3
"""Biexp.py: Simulates the response of an AFM cantilever to a biexponential force 
...
"""Known issues as of 2018_08_07:
    ...
    --Unsure if Code is currently set-up for negative phase (i.e. when drive
...
class Biexp():
    ...
    def __init__(self, exponential_rise_or_decay, tau_e, beta_e, tau_i, beta_i,
                 electronic_fraction, path = 'C:/Users/jakeprecht/DDHO/data/parameters2.cfg'):
        ...
        """when initializing this class, you will need to provide the following parameters:
            ...
            path = path to .cfg file with all the parameters.  should be same path as that used for pixel.py
        ...
        self.simulation_time = self.total_time + 20*(1/self.resonant_frequency) 
        #I add extra simulation cycles on so that I can trigger at appropriate phase and then delete some extra points so trigger is at trigger time and total time is conserved
  ...
  #      driving_tau = (np.sqrt(self.driving_force) * np.sin(self.drive_freq * t) + 
   #     np.sqrt(self.driving_force) * np.sin(self.drive_freq * 2 * t) / 2 )
    ...
    def Z_reshape_ROI_only(self, simulated_Z):
        """used for simulations to truncate data to ROI after trigger"""
        #print('len of simulated Z is', len(simulated_Z))
        
        simulation_t0_index = int(self.t0 * self.simulation_resolution)
        #print('sim t0 index is ', simulation_t0_index)
        
        simulation_offset = simulation_t0_index - self.simulation_trigger_index
        #print('sim offset is ', simulation_offset)
        #print('simulation trigger index is', self.simulation_trigger_index)
        
        #total_points = int(self.total_time*self.simulation_resolution)
        #print('total_points is', total_points)
        #This next line undoes the time shift performed bn set_t0() and cuts
        #the extraneous cycles from the end of the data
        Z_cut = simulated_Z[simulation_offset:(int((self.total_time*self.simulation_resolution) + simulation_offset)),0] 
        #print('length of z cut is ', len(Z_cut))
...
def main3(tau_e, tau_i, beta_e = 1, beta_i = 1, electronic_fraction = 0.5, color = 'r'):
    ...
    plt.plot(t_bi_reshape[3100:4500], data_bi_reshape[3100:4500],c=color, label='Cantilever z Position', lw=1,)
...
if __name__ == "__main__":
    ...
    #matrix_loop(25,'pca')
    #line_loop('row2.txt')
    #main3(100, 300, electronic_fraction = 1, color = 'r')
    main3(1, 300, electronic_fraction = 1, color = 'b')
    main3(100, 300, electronic_fraction = 1, color = 'r')
    #main3(0.01, 300, electronic_fraction = 1, color = 'c')